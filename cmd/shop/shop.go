...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		shops, err := sdkclient.FetchAll(ctx, func(ctx context.Context, page, perPage int) ([]ecommerce.Shop, bool, error) {
			path := "/shops?page=" + strconv.Itoa(page) + "&limit=" + strconv.Itoa(perPage)
...
var getCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ctx := context.Background()
		path := fmt.Sprintf("/shops/%s", args[0])
...
var createCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ctx := context.Background()
		var result ecommerce.RootShop
		_, err = sdkclient.DoRaw(ctx, httpClient, apiKey, http.MethodPost, "/shops", body, &result)
		if err != nil {
			return err
		}
...
var countCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		_, err = sdkclient.DoRaw(ctx, httpClient, apiKey, http.MethodGet, "/shops?limit=0", nil, &result)
		if err != nil {
			return err
		}
