...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		orders, err := sdkclient.FetchAll(ctx, func(ctx context.Context, page, perPage int) ([]ecommerce.Order, bool, error) {
			path := fmt.Sprintf("/shops/%s/orders?page=%d&limit=%d", shopID, page, perPage)
...
var getCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ctx := context.Background()
		path := fmt.Sprintf("/shops/%s/orders/%s", shopID, args[0])
...
var createCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ctx := context.Background()
		path := fmt.Sprintf("/shops/%s/orders", shopID)
		var result ecommerce.RootOrder
...
var countCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ctx := context.Background()
		path := fmt.Sprintf("/shops/%s/orders?limit=0", shopID)
